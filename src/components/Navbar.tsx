...
animate = {{ y: 0 }}
className = "fixed top-0 left-0 right-0 z-50 glass border-b border-pink-100 px-6 py-4"
    >
...
<div className="flex items-center gap-4" >
    <div className="hidden lg:flex items-center gap-6 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mr-4" >
        <Link to="/shop?category=accessories" className = "hover:text-primary transition-colors" > Accessories </Link>
            </div>
            < button className = "p-2 hover:text-primary transition-colors" >
                <Search className="w-5 h-5 text-gray-500" />
                    </button>
                    < a href = "https://instagram.com" target = "_blank" rel = "noopener noreferrer" className = "p-2 hover:text-primary transition-colors hidden sm:block" >
...
className = "p-2 hover:text-primary transition-colors relative"
    ...
    <ShoppingBag className="w-5 h-5 text-gray-900" />
        { cartCount > 0 && (
            <span className="absolute top-1 right-1 w-4 h-4 bg-primary text-white text-[9px] flex items-center justify-center rounded-full font-bold shadow-lg shadow-pink-100 italic" >
                { cartCount }
                </span>
              )}
</button>
